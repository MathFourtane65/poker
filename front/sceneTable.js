...
class PokerTable extends Phaser.Scene {
  ...
  creerBoutons() {
    //console.log(this);
    this.boutonMiser = this.add.text(-100, -100, "Miser", {
      fontSize: "32px",
    ...
    this.boutonMiser.on("pointerdown", () => {
      ...
      console.log("mise...");
      this.server.emit("bet", {
        seat: this.player.seat,
        amount: Number(this.print0.text),
      });
      this.updateDialogBubbleDealer(this.player.seat, Number(this.print0.text));
      this.cacherBoutons();
    });
    this.boutonSeCoucher = this.add.text(-100, -100, "Se coucher", {
      fontSize: "32px",
      backgroundColor: "#87cefa",
      color: "#000",
    });
    ...
    this.boutonResetGame = this.add.text(-100, -100, "RESTART", {
      fontSize: "32px",
      backgroundColor: "#87cefa",
      color: "#000",
    });
    this.boutonResetGame.setInteractive({ useHandCursor: true });
    this.boutonResetGame.on("pointerdown", () => {
  ...
  create() {
    ...
    let aaa = (this.print0 = this.add.text(1345, 40, "", {
      color: "black",
      fontSize: "30px ",
    }));
    this.bbb = this.rexUI.add
      .slider({
        x: 1335,
        y: 90,
        width: 200,
    ...
    this.text = this.add.text(200, 50, "Entrer votre pseudo SVP", {
      color: "black",
      fontSize: "30px ",
    });
    var text = this.text;
  ...
  createCard(card, scale) {
    let suitIdx = 4,
      rankIdx = 0; //blue cover idx in spritesheet
    if (card) {
  ...
  dealOpenCards(seat, cards, isPocketCards) {
    ...
    this.dealCard(card1, this.seats[seat].x, this.seats[seat].y, () => {
      ...
      this.dealCard(
        card2,
        this.seats[seat].x + cardParams.width * scale * 1.2,
        this.seats[seat].y
      );
    });
    ...
    if (
      this.seats[seat].x === this.seats[1].x &&
  ...
  dealClosedCards(seat, isPocketCards) {
    ...
    this.dealCard(card1, this.seats[seat].x, this.seats[seat].y, () => {
      ...
      this.dealCard(
        card2,
        this.seats[seat].x + cardParams.width * scale,
        this.seats[seat].y,
        () => {
          console.log("play...");
        }
  ...
  afficherBoutons() {
    ...
    // this.boutonResetGame.x = 10;
    // this.boutonResetGame.y = 10;
